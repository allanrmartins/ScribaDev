...
> 🎙️ Automatic recording, transcription and minutes for your meetings — Teams, Zoom, Google Meet and browser friends — 100% local and private. **Free while in beta.**
...
<p align="center">
  ...
  <br><br>
  <img src="docs/janela.png" alt="Scriba — main window with service status" width="480">
  <br><br>
  <img src="docs/notas.png" alt="Scriba — notes reader with titles" width="780">
</p>

Scriba lives in the Windows tray and **detects by itself when you join a Teams or Zoom call — or a meeting in the browser** (Google Meet, Teams web, Zoom web…). It records the audio (no bot ever joins the meeting), transcribes locally with Whisper on your GPU/CPU and produces a Markdown file **with a title, client and structured summary** (participants, decisions, requirements, SAP objects mentioned, action items) plus the **full transcript**, with turns attributed to **Me** (your microphone) and **Participants** (the audio you hear) — or **Participante 1/2/3** with optional diarization. Think of it as a minimalist, open-source, homemade "Granola" — built for people who feed meeting notes into development tools such as Claude Code.
...
claude -p (optional) ──► meeting title + client + structured summary
        │
        ▼
notas.md ──► exported to Documents\Scriba\
        │
        ▼
audio archived as Opus (~20 MB/h) ──► folder renamed with the note's title
```

- **Detection without APIs**: Windows tracks when Teams/Zoom opens the microphone (`HKCU\...\CapabilityAccessManager\ConsentStore\microphone`). Scriba just watches that key — any account, no Graph API, no admin rights.
...
Installation is **automatic**: `setup.ps1` does everything — creates an isolated Python environment, installs dependencies (with CUDA support if an NVIDIA GPU is present, ~1.3 GB), downloads the Whisper model (~1.6 GB on first run), puts the `scriba` command on your PATH and finishes by running `scriba doctor`, which checks GPU, Teams, audio, model and folders.
...
|---|---|---|
| Final notes (`.md`) | `Documents\Scriba\` | ✅ Settings window |
| Recordings (one folder per meeting, in a date tree named after the note's title: `2026\06\12\16-34_Boleto não gerado em produção\`) | `C:\temp\scriba\gravacoes\` | ✅ Settings window |
| Summary prompt (`prompt.md`) | `%LOCALAPPDATA%\Scriba\prompt.md` | ✅ Summary tab (built-in editor) |
| Config, logs and Python environment | `%LOCALAPPDATA%\Scriba\` | — |
...
```powershell
scriba run            # start monitoring (tray icon)
scriba autostart on   # optional: start with Windows
```
...
| Command | Purpose |
|---|---|
| `scriba run` | tray app: detection + recording + processing |
| `scriba doctor` | environment diagnostic (`--toast` tests notifications) |
| `scriba devices` | list available microphones and loopbacks |
| `scriba record 60` | manual N-second recording (e.g. meetings outside Teams) |
| `scriba transcribe <folder>` | (re)transcribe a meeting (`--cpu` forces CPU) |
| `scriba summarize <folder>` | (re)generate the summary and notas.md |
| `scriba process <folder>` | everything at once: transcribe + summarize + export |
| `scriba detect` | debug: print detection state transitions live |
| `scriba wizard` | profile assistant: generates prompt.md and hotwords for your role |
| `scriba autostart on\|off` | shortcut in the Windows Startup folder |
| `scriba shortcut` | (re)create the Desktop and Start Menu shortcuts |
| `scriba purge` | delete already-transcribed recordings past the retention window (`--days N` overrides, `--dry-run` lists only) |
...
[audio]
loopback_device = ""     # empty = default output; otherwise part of the name (scriba devices)
...
[output]
export_dir = ""          # empty = Documents\Scriba
recordings_dir = ""      # empty = C:\temp\scriba\gravacoes (created automatically)
```

Scriba defaults to Brazilian Portuguese, but `language`, `hotwords` and the summary prompt work with anything Whisper supports.
...
1. In Scriba's venv: `pip install torch --index-url https://download.pytorch.org/whl/cu128` (no NVIDIA GPU: just `pip install torch`) then `pip install pyannote.audio`;
...
- Recordings and transcripts live in `C:\temp\scriba\gravacoes` (configurable), away from synced folders — and are **deleted automatically** after the retention window (default 30 days; `0` keeps forever); config and logs in `%LOCALAPPDATA%\Scriba`. Only the final `.md` goes to Documents.

## Legal notice

Scriba records **locally** the audio entering and leaving your machine, without notifying the other participants (no bot joins the call). Call-recording laws vary by country, and company/client policies may require explicit consent. **Check the rules that apply to you before using it — you are responsible for how you use this tool.**
...
- Summaries via other providers (local Ollama, OpenAI)

## Contributing

Bug reports and suggestions via issues are very welcome — run `scriba doctor` before filing and include its output. The project does **not accept code pull requests** for now: Scriba is transitioning to a subscription product, and external contributions would complicate licensing. The codebase remains small and modular — one responsibility per file: `detector`, `recorder`, `transcriber`, `diarize`, `merge`, `notes`, `overlay`, `main_window`, `notes_ui`, `settings_ui`, `tray`, `main` — and open for reading and auditing.

## License

Scriba is **source-available** under the [Elastic License 2.0](LICENSE) © Allan Martins. The code stays open for reading and auditing — transparency matters in an app that records your meetings — and it is **free to use while in beta**, including at work. The license does not allow: offering Scriba to third parties as a product or service, working around license-key functionality, or removing license notices. At go-live, Scriba becomes a subscription product.
